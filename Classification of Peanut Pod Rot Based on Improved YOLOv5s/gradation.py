...
exp_folders = [folder for folder in os.listdir(detect_folder) if folder.startswith("exp")]
...
for file_name in os.listdir(location_center_folder):
    if file_name.endswith(".txt"):
        ...
        # 将临时DataFrame合并到结果DataFrame中
        result_df = pd.concat([result_df, temp_df])

# 指定保存Excel文件的路径，保存到根目录下
excel_path = os.path.join(yolov5_root, "result.xlsx")
